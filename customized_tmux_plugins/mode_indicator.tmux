...
init_tmux_mode_indicator() {
  local -r \
    prefix_prompt=$(tmux_option "$prefix_prompt_config" " ") \
    copy_prompt=$(tmux_option "$copy_prompt_config" " ") \
    sync_prompt=$(tmux_option "$sync_prompt_config" " ") \
    empty_prompt=$(tmux_option "$empty_prompt_config" " ") \
    prefix_style=$(indicator_style "$prefix_mode_style_config" "fg=cyan") \
    copy_style=$(indicator_style "$copy_mode_style_config" "fg=#105BCA") \
    sync_style=$(indicator_style "$sync_mode_style_config" "fg=yellow") \
    empty_style=$(indicator_style "$empty_mode_style_config" "fg=red")
